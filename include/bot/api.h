...
public:
	...
	Api(std::string_view url, std::string_view bot_token);
private:
	AsyncHttpClient async_http_client;

public:	
	Message::Ptr sendMessage(std::string_view chat_id,
	...
	Message::Ptr forwardMessage(std::string_view chat_id,
	...
	bool editMessageMedia(std::string_view chat_id,
						  int message_id,
						  std::string_view inline_message_id = "",
						  const GenericMedia::Ptr& media = nullptr,
						  const InlineKeyboardMarkup::Ptr& reply_markup = nullptr) const;
